...
이 명령은 현재 theme를 읽고 syntax, diagnostics, diff 샘플을 Webview로 엽니다. 샘플 영역을 클릭하면 Webview가 클릭 맥락을 담은 `CalibrationIntent`를 Extension Host로 전달합니다. Extension Host는 현재 theme risk를 기준으로 관련 solution candidate를 생성하고, 생성 결과를 상태 알림으로 표시합니다.
